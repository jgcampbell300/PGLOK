...
def test_pglok_integration():
    """Test if PGLOK can import the dependency checker hook."""
    print("\n🧪 Testing PGLOK integration...")

    try:
        pglok_path = Path(__file__).parent / "src" / "pglok.py"
        if pglok_path.exists():
            with open(pglok_path, "r", encoding="utf-8") as f:
                content = f.read()
            ...
            if "from simple_dependency_checker import safe_show_dependency_checker" in content:
                print("✅ safe_show_dependency_checker import found in PGLOK")
            else:
                print("❌ safe_show_dependency_checker import not found in PGLOK")
                return False
        else:
